...
@section('content')
    <div id="room-carousel" class="flex justify-center w-full">
        <div class="carousel h-80 overflow-x-auto" style="width: 75dvw;">
            ...
            >
                @foreach($avgMeasurements->groupBy('room_id') as $roomId => $entries)
                    <a href="{{ route('dashboard.room', ['id' => $roomId]) }}" class="carousel-slide flex-shrink-0 w-64 sm:w-80 md:w-96 p-2 snap-center">
                        ...
                        <span class="text-2xl sm:text-4xl text-center">
                            {{ $entries->first()->house }} ({{ $entries->first()->room }})
                        </span>
                        </div>
                    </a>
                @endforeach
